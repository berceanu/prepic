...
def test_w0_to_fwhm():
    ...
    fwhm = lwfa.w0_to_fwhm(7.202530529256849*u.micrometer)
    assert_allclose_units(fwhm, 8.4803316326207*u.micrometer)
...
def test_plasma_with_laser():
    ...
    assert_allclose_units(plasma.Pc.to_value('terawatt'), 100.72555, 4)

    assert plasma.depletion.units.dimensions == dimensions.length
    assert_allclose_units(plasma.depletion.to_value('micrometer'), 71051.20374, 4)

    assert plasma.dephasing.units.dimensions == dimensions.length
    assert_allclose_units(plasma.dephasing.to_value('micrometer'), 118514.39895, 4)
